...
const PSWProfileDisplayPage = () => {
  ...
  const [payload, setPayload] = useState(() => {
    if (location.state?.profile)
      return { profile: location.state.profile, certificates: [], reviews: [] };
    return null;
  });
      ...
      <PageTransition className="mx-auto w-full max-w-6xl space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="page-subtitle text-sm">
                  Review details before booking care services.
                </p>
              </div>
            </div>
            {profile ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="space-y-4">
            ...
            {profile && activeTab === "availability" ? (
              <section className="app-card">
                ...
                {profile.availability?.length > 0 ? (
                  <div className="mt-4 space-y-2">
                    {profile.availability.map((slot) => (
                      <div
                        className="flex items-center justify-between rounded-xl border border-brand-100/60 bg-brand-50/20 px-4 py-2.5"
                ...
                <div className="mt-4 space-y-3">
                  {certificates.length > 0 ? (
                    certificates.map((cert) => (
                      <article
                        className="rounded-xl border border-brand-100/60 bg-brand-50/20 px-4 py-3 flex items-center justify-between"
                        key={cert._id}
                      >
                        <p className="text-sm font-medium text-slate-800">
                          {cert.originalFileName || "Certificate"}
                        </p>
                    ...
                    <EmptyState
                      ...
                      description="No certificates available for this provider."
                    />
                  )}
                </div>
              </section>
            ...
            {profile && activeTab === "reviews" ? (
              <section className="app-card">
                ...
                {currentRole === "client" ? (
                  <form
                    className="mt-4 space-y-3 rounded-xl border border-brand-100/60 bg-brand-50/20 p-4"
                  ...
                  >
                    ...
                    {eligibleAppointments.length > 0 ? (
                      ...
                      <p className="text-xs text-slate-500">
                        Only available for completed, unreviewed appointments.
                      </p>
                    )}
                    <textarea
                    ...
                    />
                    {reviewError ? (
                      <ErrorBanner message={reviewError} compact />
                    ) : null}
                    {reviewSuccess ? (
                ...
                <div className="mt-4 space-y-3">
                  {reviews.length > 0 ? (
                    reviews.map((review) => (
                      <article
                        className="rounded-xl border border-brand-100/60 bg-white px-4 py-3"
                        key={review._id}
                      >
